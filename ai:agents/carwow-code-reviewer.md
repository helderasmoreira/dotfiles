...
- **What the diff doesn't show** — the worst misses hide in absences. When code is moved, extracted, or restructured (controller split, page → turbo frame, shared partial/bundle), reconstruct what the old path did — before_actions, layout options, guards, analytics events, redirects — and confirm each carried over or was dropped deliberately. Apply the same instinct to effects outside the visible lines: selectors in shared CSS bundles, other callers of a changed method, and gem behaviour behind a config option (read the gem source rather than trusting the option's name).
